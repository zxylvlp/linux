 ...
 * delta /= w
 */
// 计算公平调度的delta，参数是delta和调度单元
static inline u64 calc_delta_fair(u64 delta, struct sched_entity *se)
{
    // 如果调度单元的权重不等于nice0load
	if (unlikely(se->load.weight != NICE_0_LOAD))
        // 以用delta和nice0load为基准，计算当前调度单元load对应的delta
...
// tick当前调度单元，参数是cfsrq，当前调度单元和是否是精准tick
static void
entity_tick(struct cfs_rq *cfs_rq, struct sched_entity *curr, int queued)
{
	 ...
	 */
    // 如果cfs_rq的rq打开了hrtimer并且没有两重tick特性则直接返回
...
wakeup_preempt_entity(struct sched_entity *curr, struct sched_entity *se)
{
    // 将当前任务的运行时间减去指定任务的运行时间
	s64 gran, vdiff = curr->vruntime - se->vruntime;

    // 如果当前任务运行时间更少则返回-1表示不能被抢占
	if (vdiff <= 0)
		return -1;

    // 获得唤醒保证
	gran = wakeup_gran(se);
    // 如果指定任务的运行时间更少的超过保证则返回应该抢占
	if (vdiff > gran)
		return 1;
 ...
 */
// 公平调度器的任务被tick之后的回调函数, 参数是运行队列，当前任务和是否是精准tick
static void task_tick_fair(struct rq *rq, struct task_struct *curr, int queued)
{
    ...
    // 向父级遍历遍历当前任务的所有调度单元
	for_each_sched_entity(se) {
        // 获得调度单元的cfsrq，并且对对应的cfsrq进行tick
		cfs_rq = cfs_rq_of(se);
